...
function captcha_html($class="captcha")
{
    ...
    $html .= "\n".'<script>var gml_captcha_url  = "'.GML_CAPTCHA_URL.'";</script>';
    //$html .= "\n".'<script>var gml_captcha_path = "'.GML_CAPTCHA_PATH.'";</script>';
    $html .= "\n".'<script src="'.GML_CAPTCHA_URL.'/kcaptcha.js"></script>';
    $html .= "\n".'<fieldset id="captcha" class="'.$class.'">';
    $html .= "\n".'<legend><label for="captcha_key">'.__('Captcha').'</label></legend>';
    if (is_mobile()) $html .= '<audio id="captcha_audio" controls></audio>';
    //$html .= "\n".'<img src="#" alt="" id="captcha_img">';
    $html .= "\n".'<img src="'.GML_CAPTCHA_URL.'/img/dot.gif" alt="" id="captcha_img">';
    $html .= '<input type="text" name="captcha_key" id="captcha_key" required class="captcha_box required" size="6" maxlength="6">';
    if (!is_mobile()) $html .= "\n".'<button type="button" id="captcha_mp3"><span></span>'.__('Listen to numeric voices').'</button>';
    $html .= "\n".'<button type="button" id="captcha_reload"><span></span>'.__('Refresh').'</button>';
    $html .= "\n".'<span id="captcha_info">'.__('Please enter captcha numbers in order.').'</span>';
    $html .= "\n".'</fieldset>';
    return $html;
}
