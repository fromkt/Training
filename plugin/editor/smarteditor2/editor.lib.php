...
{
    global $gml, $config, $w, $board, $write;
    static $js = true;
    ...
    if ($is_dhtml_editor && $js) {
        ...
        $html .= "\n".'<script>var gml_editor_url = "'.$editor_url.'", oEditors = [], ed_nonce = "'.ft_nonce_create('smarteditor').'";</script>';
    ...
    $html .= "\n<textarea id=\"$id\" name=\"$id\" class=\"$smarteditor_class\" maxlength=\"65536\" style=\"width:100%;min-width:260px;height:300px\">$content</textarea>";
...
{
    if ($is_dhtml_editor) {
        return "if (!{$id}_editor_data || jQuery.inArray({$id}_editor_data.toLowerCase(), ['&nbsp;','<p>&nbsp;</p>','<p><br></p>','<p></p>','<br>']) != -1) { alert(\"".__('Enter contents.')."\"); oEditors.getById['{$id}'].exec('FOCUS'); return false; }\n";
    } else {
        return "if (!{$id}_editor.value) { alert(\"".__('Enter contents.')."\"); {$id}_editor.focus(); return false; }\n";
    }
}
...
if (!defined('FT_NONCE_UNIQUE_KEY'))
    define( 'FT_NONCE_UNIQUE_KEY' , sha1($_SERVER['SERVER_SOFTWARE'].GML_MYSQL_USER.session_id().GML_TABLE_PREFIX) );
