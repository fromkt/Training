...
jQuery(function ($) {
	...
	var listeners = {
		...
		fileQueueError: function(event, file, errorCode, message){
            ...
            {
                case -100 :
                    alert(sprintf(supload.select_file_msg, message) );
                    break;
            }
    ...
    $(gnu.dreg_area).bind('drop dragover', function (e) {
        e.preventDefault();
        if( !gnu.file_api_support && e.type == 'drop' ) alert( supload.is_ie_msg );
    });
    $("#all_remove_btn").bind("click", function(e){
        e.preventDefault();
        if( $(gnu.dreg_area_list).children().length ){
            if (confirm( supload.delete_msg )){
