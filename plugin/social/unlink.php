...
if($mp_no){
    $sql = "SELECT * from {$gml['social_profile_table']} where mb_id= '".$mb_id."' and mp_no= $mp_no";
    $row = sql_fetch($sql);
} else if($provider){
    $sql = "SELECT * from {$gml['social_profile_table']} where mb_id= '".$mb_id."' and provider= '".$provider."'";
    $row = sql_fetch($sql);
}
