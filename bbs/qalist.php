...
if ($qaconfig['qa_category']) {
    $category_href = GML_BBS_URL.'/qalist.php';
...
if(is_file($skin_file)) {
    $sql_common = " from {$gml['qa_content_table']} ";
    ...
    $page_rows = GML_IS_MOBILE ? $qaconfig['qa_mobile_page_rows'] : $qaconfig['qa_page_rows'];
    $total_page  = ceil($total_count / $page_rows);  // Get total pages 전체 페이지 계산
    if ($page < 1) { $page = 1; } // if page == 0 then page value is 1, 페이지가 없으면 첫 페이지 (1 페이지)
    $from_record = ($page - 1) * $page_rows; // Get sql start on record, 시작 열을 구함
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $list[$i]['view_href'] = GML_BBS_URL.'/qaview.php?qa_id='.$row['qa_id'].$qstr;
    ...
    if($is_admin) {
        $is_checkbox = true;
        $admin_href = GML_ADMIN_URL.'/qa_config.php';
    }

    $list_href = GML_BBS_URL.'/qalist.php';
    $write_href = GML_BBS_URL.'/qawrite.php';

    $list_pages = preg_replace('/(\.php)(&amp;|&)/i', '$1?', get_paging(GML_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './qalist.php'.$qstr.'&amp;page='));
    ...
    if ($is_admin) {
        include_once(GML_THEME_JS_PATH. '/qa_list_js.php');
    }
} else {
