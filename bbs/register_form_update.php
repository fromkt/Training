...
if ($w == 'u' && $is_admin == 'super') {
    if (file_exists(GML_PATH.'/DEMO'))
        alert(__('You can not do this on the demo screen.'));
}
...
if($w == 'u')
    ...
    alert(__('Its a wrong approach.'), GML_URL);

if(!$mb_id)
    alert(sprintf(__('%s parameter is wrong'), 'mb_id').' '.__('Please use the correct method.'));
...
$mb_country     = isset($_POST['mb_country'])       ? trim($_POST['mb_country'])     : "";
...
if ($w == '' || $w == 'u') {
    ...
    if($tmp_mb_name != $mb_name) {
        alert(__('Please enter a valid name.'));
    }
    $tmp_mb_nick = iconv('UTF-8', 'UTF-8//IGNORE', $mb_nick);
    if($tmp_mb_nick != $mb_nick) {
        alert(__('Please enter a valid nickname.'));
    }

    if ($w == '' && !$mb_password)
        alert(__('Password value is empty.'));
    if($w == '' && $mb_password != $mb_password_re)
        alert(__('The passwords do not match.'));
    ...
    if ($w=='') {
        ...
        if ($config['cf_use_recommend'] && $mb_recommend) {
            if (!exist_mb_id($mb_recommend))
                alert(__('Recommendation does not exist.'));
        }

        if (strtolower($mb_id) == strtolower($mb_recommend)) {
            alert(__('You can not recommend yourself.'));
        }
    } else {
        // 자바스크립트로 정보변경이 가능한 버그 수정
        // 닉네임수정일이 지나지 않았다면
        if ($member['mb_nick_date'] > date("Y-m-d", GML_SERVER_TIME - ($config['cf_nick_modify'] * 86400)))
...
if($config['cf_cert_use'] && $_SESSION['ss_cert_type'] && $_SESSION['ss_cert_dupinfo']) {
    // 중복체크
    $sql = " select mb_id from {$gml['member_table']} where mb_id <> '{$member['mb_id']}' and mb_dupinfo = '{$_SESSION['ss_cert_dupinfo']}' ";
    $row = sql_fetch($sql);
    if ($row['mb_id']) {
        alert(__('The details registered with the identification information you entered exist.').'\\n'.__('Member ID').' : '.$row['mb_id']);
    }
}
...
if ($w == '') {
    ...
    if (!$config['cf_use_email_certify'])
        $sql .= " , mb_email_certify = '".GML_TIME_YMDHIS."' ";
    sql_query($sql);
    ...
    // 추천인에게 포인트 부여
    if ($config['cf_use_recommend'] && $mb_recommend)
        insert_point($mb_recommend, $config['cf_recommend_point'], sprintf(__('Recommendation for %s'), $mb_id), '@member', $mb_recommend, $mb_id.' recommend');

    // 회원님께 메일 발송
    if ($config['cf_email_mb_member']) {
        $subject = apply_replace('register_form_update_mail_mb_subject', '['.$config['cf_title'].'] '.__('Congratulations on your membership.'), $mb_id);
    ...
    if (!trim($_SESSION['ss_mb_id']))
        alert(__('You are not logged in.'));

    if (trim($_POST['mb_id']) != $mb_id)
        alert(__('You can not modify the logged in information because the information you are trying to modify is not correct.')."\\n".__('Stop immediately if incorrect method is used.'));
    ...
    if ($mb_nick_default != $mb_nick)
        $sql_nick_date =  " , mb_nick_date = '".GML_TIME_YMD."' ";

    $sql_open_date = "";
    if ($mb_open_default != $mb_open)
        $sql_open_date =  " , mb_open_date = '".GML_TIME_YMD."' ";
...
if (isset($_FILES['mb_icon']) && is_uploaded_file($_FILES['mb_icon']['tmp_name'])) {
    if (preg_match($image_regex, $_FILES['mb_icon']['name'])) {
        ...
        if ($_FILES['mb_icon']['size'] <= $config['cf_member_icon_size']) {
            ...
            $msg .= sprintf(__('Upload member icon to %s bytes or less.'), number_format($config['cf_member_icon_size']));
        }

    } else {
        $msg .= sprintf(__('% is not an image file.'), $_FILES['mb_icon']['name']);
    }
}

// 회원 프로필 이미지
if( $config['cf_member_img_size'] && $config['cf_member_img_width'] && $config['cf_member_img_height'] ){
    $mb_tmp_dir = GML_DATA_PATH.'/member_image/';
    $mb_dir = $mb_tmp_dir.substr($mb_id,0,2);
    if( !is_dir($mb_tmp_dir) ){
        @mkdir($mb_tmp_dir, GML_DIR_PERMISSION);
        @chmod($mb_tmp_dir, GML_DIR_PERMISSION);
    }
    ...
    if (isset($_FILES['mb_img']) && is_uploaded_file($_FILES['mb_img']['tmp_name'])) {
        ...
        if (preg_match($image_regex, $_FILES['mb_img']['name'])) {
            ...
            if ($_FILES['mb_img']['size'] <= $config['cf_member_img_size']) {
                ...
                $msg .= sprintf(__('Upload member image to %s bytes or less.'), number_format($config['cf_member_img_size']));
            }

        } else {
            $msg .= sprintf(__('%s is not an gif/jpg file.'), $_FILES['mb_img']['name']);
        }
    }
}
...
if ($w == '') {
    ...
    $row  = get_member($member['mb_id']);
    $tmp_password = $row['mb_password'];

    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        set_session('ss_mb_id', '');
        alert(__('Member profile has been modified.').'\n\n'.__('E-mail address has been changed, so you need to authenticate again.'), GML_URL);
    } else {
        echo '
        <!doctype html>
        <html lang="ko">
        <head>
        <meta charset="utf-8">
        <title>'.__('Edit Member Profile').'</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.GML_HTTP_BBS_URL.'/register_form.php">
