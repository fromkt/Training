...
$sql = " select gr_id, bo_subject, bo_page_rows, bo_read_level, bo_use_rss_view from {$gml['board_table']} where bo_table = '$bo_table' ";
$row = sql_fetch($sql);
...
if ($row['bo_read_level'] >= 2) {
    e__('Only Boards that can read non-members support RSS.');
    exit;
}

// RSS 사용 체크
if (!$row['bo_use_rss_view']) {
    _('The RSS view is disable.');
    exit;
}
...
$sql = " select gr_subject from {$gml['group_table']} where gr_id = '{$row['gr_id']}' ";
$row = sql_fetch($sql);
...
<description>test version 0.2 (2004-04-26)</description>
<language>ko</language>

<?php
$sql = " select wr_id, wr_subject, wr_content, wr_name, wr_datetime, wr_option
            from {$gml['write_prefix']}$bo_table
