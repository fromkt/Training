...
if(strpos($filename, 'data/editor')) {
    ...
    $filepath = GML_DATA_PATH.'/'.$editor_file;
} else if(strpos($filename, 'data/qa')) {
    $editor_file = strstr($filename, 'qa');
    $filepath = GML_DATA_PATH.'/'.$editor_file;
} else {
    $editor_file = '';
    $filepath = GML_DATA_PATH.'/file/'.$bo_table.'/'.$filename;
}

if(is_file($filepath)) {
    $size = @getimagesize($filepath);
    if(empty($size))
        alert_close(__('Not an image file.'));

    $width = $size[0];
    $height = $size[1];

    if($editor_file)
        $fileurl = GML_DATA_URL.'/'.$editor_file;
    else
        $fileurl = GML_DATA_URL.'/file/'.$bo_table.'/'.$filename;

    $img = '<img src="'.$fileurl.'" alt="" width="'.$width.'" height="'.$height.'" class="draggable" style="position:relative;top:0;left:0;cursor:move;">';
} else {
    alert_close(__('File does not exist.'));
}
?>
