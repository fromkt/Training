...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    if ($row['mb_id']) {
        ...
        if ($is_admin)
            ...
            $list[$i]['name'] = preg_replace("/([0-9]+).([0-9]+).([0-9]+).([0-9]+)/", GML_IP_DISPLAY, $row['lo_ip']);
    }
...
bind_lang_domain( 'default', get_path_lang_dir('skin', $connect_skin_path.'/'.GML_LANG_DIR) );
