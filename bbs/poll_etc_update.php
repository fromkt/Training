...
if ($w == '')
{
    ...
    $po = sql_fetch(" select * from {$gml['poll_table']} where po_id = '{$po_id}' ");
    if (!$po['po_id'])
        alert(sprintf(__('%s parameter is wrong'), 'po_id'));

    $tmp_row = sql_fetch(" select max(pc_id) as max_pc_id from {$gml['poll_etc_table']} ");
    $pc_id = $tmp_row['max_pc_id'] + 1;

    $sql = " insert into {$gml['poll_etc_table']}
                ( pc_id, po_id, mb_id, pc_name, pc_idea, pc_datetime )
                values ( '{$pc_id}', '{$po_id}', '{$member['mb_id']}', '{$pc_name}', '{$pc_idea}', '".GML_TIME_YMDHIS."' ) ";
    sql_query($sql);
    ...
    if ($config['cf_email_po_super_admin'])
    {
        ...
        mailer($name, $from_email, $admin['mb_email'], '['.$config['cf_title'].'] '.__('Other opinions from Poll'), $content, 1);
    }
}
else if ($w == 'd')
{
    ...
    {
        $sql = " delete from {$gml['poll_etc_table']} where pc_id = '{$pc_id}' ";
        if (!$is_admin)
