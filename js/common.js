...
function del(href)
{
    if(confirm( common_js_l10n.confirm_delete_text )) {
        var iev = -1;
...
var win_zip = function(frm_name, frm_zip, frm_addr1, frm_addr2, frm_addr3) {
    if(typeof daum === 'undefined'){
        alert("postcode.v2.js file not load");
        return false;
    }
    ...
    var complete_fn = function(data){
        ...
        if(of[frm_addr3] !== undefined){
            of[frm_addr3].value = data.userSelectedType;
        }
    ...
    switch(zip_case) {
        case 1 :    //iframe을 이용하여 페이지에 끼워 넣기
            ...
            new daum.Postcode({
                ...
                maxSuggestItems : gml_is_mobile ? 6 : 10,
                width : '100%',
                height : '100%'
            }).embed(element_wrap);
...
function font_resize(id, rmv_class, add_class, othis)
{
    ...
    set_cookie("ck_font_resize_rmv_class", rmv_class, 1, gml_cookie_domain);
    set_cookie("ck_font_resize_add_class", add_class, 1, gml_cookie_domain);
...
function set_comment_token(f)
{
    if(typeof f.token === "undefined")
        jQuery(f).prepend('<input type="hidden" name="token" value="">');

    jQuery.ajax({
        url: gml_bbs_url+"/ajax.comment_token.php",
        type: "GET",
        dataType: "json",
        async: false,
        cache: false,
...
jQuery(function($){
    ...
    */
    ...
    $(document).on("click", "form[name=fwrite] input:submit, form[name=fwrite] button:submit, form[name=fwrite] input:image", function() {
        ...
        if(!token) {
            alert( common_js_l10n.token_wrong_text );
            return false;
        }
...
function get_write_token(bo_table)
{
    ...
    jQuery.ajax({
        type: "POST",
        url: gml_bbs_url+"/write_token.php",
