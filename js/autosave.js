...
function autosave() {
    jQuery("form#fwrite").each(function() {
        if(gml_editor != "") {
            if (gml_editor.indexOf("ckeditor4") != -1 && typeof(CKEDITOR.instances.wr_content)!="undefined") {
                this.wr_content.value = CKEDITOR.instances.wr_content.getData();
            } else if (gml_editor.indexOf("cheditor5") != -1 && typeof(ed_wr_content)!="undefined") {
        ...
        if (save_wr_subject != this.wr_subject.value || save_wr_content != this.wr_content.value) {
            jQuery.ajax({
                url: gml_bbs_url+"/ajax.autosave.php",
                data: {
                    "uid" : this.uid.value,
...
jQuery(function($){
    ...
    $("#btn_autosave").click(function(){
        if ($("#autosave_pop").is(":hidden")) {
            $.get(gml_bbs_url+"/ajax.autosavelist.php", function(data){
                //alert(data);
                ...
                if ($(data).find("list").find("item").length > 0) {
                    $(data).find("list").find("item").each(function(i) {
                        ...
                        $("#autosave_pop ul")
                            .append('<li><a href="#none" class="autosave_load">'+subject+'</a><span>'+datetime+' <button type="button" class="autosave_del"><span class="sound_only">삭제</span><i class="fa fa-trash-o" aria-hidden="true"></i></button></span></li>')
                            .find("li:eq("+i+")")
    ...
    $(document).on( "click", ".autosave_load", function(){
        ...
        $.get(gml_bbs_url+"/ajax.autosaveload.php", {"as_id":as_id}, function(data){
    ...
    $(document).on( "click", ".autosave_del", function(){
        var $li = $(this).parents("li");
        var as_id = $li.data("as_id");
        $.get(gml_bbs_url+"/ajax.autosavedel.php", {"as_id":as_id}, function(data){
            if (data == -1) {
                alert( autosave_js_l10n.error_text );
            } else {
                $("#autosave_count").html(data);
                $li.remove();
            }
