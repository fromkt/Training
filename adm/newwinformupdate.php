...
if($w == "")
{
    $sql = " insert {$gml['new_win_table']} set $sql_common ";
    sql_query($sql);
...
{
    $sql = " update {$gml['new_win_table']} set $sql_common where nw_id = '$nw_id' ";
    sql_query($sql);
}
else if ($w == "d")
{
    $sql = " delete from {$gml['new_win_table']} where nw_id = '$nw_id' ";
    sql_query($sql);
}
