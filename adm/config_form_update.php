...
if (!$mb['mb_id'])
    alert(__('The super administrator member ID does not exist.'));

check_admin_token();

// 본인확인을 사용할 경우 아이핀, 휴대폰인증 중 하나는 선택되어야 함
if($_POST['cf_cert_use'] && !$_POST['cf_cert_ipin'] && !$_POST['cf_cert_hp'])
    alert(__('Please select at least one I-PIN or mobile phone Authentication service for identification'));
...
$sql = " update {$gml['config_table']}
            set cf_title = '{$_POST['cf_title']}',
                ...
                cf_notice_del = '{$_POST['cf_notice_del']}',
...
//sql_query(" OPTIMIZE TABLE `$gml[config_table]` ");
