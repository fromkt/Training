...
if (!preg_match('/[A-Za-z0-9_]{1,20}/', $target_table)) {
    alert(__('Board table available only letters, numbers, _ with no spaces. (20 characters)'));
}

// 게시판명이 금지된 단어로 되어 있으면
if ( $w == '' && in_array($target_table, get_bo_table_banned_word()) ){
    alert(__('Board TABLE Name can not be used. Please use a different name.'));
}

$row = sql_fetch(" select count(*) as cnt from {$gml['board_table']} where bo_table = '$target_table' ");
if ($row['cnt'])
    alert(sprintf(__('%s is a board table names that already exist'), $target_table).'\\n'.__('Can not be used as the name of the table to copy.'));

// Create Board table , 게시판 테이블 생성
$sql = get_table_define($gml['write_prefix'] . $bo_table);
$sql = str_replace($gml['write_prefix'] . $bo_table, $gml['write_prefix'] . $target_table, $sql);
sql_query($sql, false);
...
// 게시판 정보
$sql = " insert into {$gml['board_table']}
...
// 게시판 폴더 생성
@mkdir(GML_DATA_PATH.'/file/'.$target_table, GML_DIR_PERMISSION);
@chmod(GML_DATA_PATH.'/file/'.$target_table, GML_DIR_PERMISSION);

// 디렉토리에 있는 파일의 목록을 보이지 않게 한다.
$board_path = GML_DATA_PATH.'/file/'.$target_table;
...
if ($copy_case == 'schema_data_both') {
    $d = dir(GML_DATA_PATH.'/file/'.$bo_table);
    while ($entry = $d->read()) {
        if ($entry == '.' || $entry == '..') continue;

        // 김선용 201007 :
        if(is_dir(GML_DATA_PATH.'/file/'.$bo_table.'/'.$entry)){
            $dd = dir(GML_DATA_PATH.'/file/'.$bo_table.'/'.$entry);
            @mkdir(GML_DATA_PATH.'/file/'.$target_table.'/'.$entry, GML_DIR_PERMISSION);
            @chmod(GML_DATA_PATH.'/file/'.$target_table.'/'.$entry, GML_DIR_PERMISSION);
            while ($entry2 = $dd->read()) {
                if ($entry2 == '.' || $entry2 == '..') continue;
                @copy(GML_DATA_PATH.'/file/'.$bo_table.'/'.$entry.'/'.$entry2, GML_DATA_PATH.'/file/'.$target_table.'/'.$entry.'/'.$entry2);
                @chmod(GML_DATA_PATH.'/file/'.$target_table.'/'.$entry.'/'.$entry2, GML_DIR_PERMISSION);
                $copy_file++;
            }
            $dd->close();
        }
        else {
            @copy(GML_DATA_PATH.'/file/'.$bo_table.'/'.$entry, GML_DATA_PATH.'/file/'.$target_table.'/'.$entry);
            @chmod(GML_DATA_PATH.'/file/'.$target_table.'/'.$entry, GML_DIR_PERMISSION);
            $copy_file++;
        }
    }
    ...
    // 글복사
    $sql = " insert into {$gml['write_prefix']}$target_table select * from {$gml['write_prefix']}$bo_table ";
    sql_query($sql, false);

    // 게시글수 저장
    $sql = " select bo_count_write, bo_count_comment from {$gml['board_table']} where bo_table = '$bo_table' ";
    $row = sql_fetch($sql);
    $sql = " update {$gml['board_table']} set bo_count_write = '{$row['bo_count_write']}', bo_count_comment = '{$row['bo_count_comment']}' where bo_table = '$target_table' ";
    sql_query($sql, false);

    // 4.00.01
    // 위의 코드는 같은 테이블명을 사용하였다는 오류가 발생함. (희한하네 ㅡㅡ;)
    $sql = " select * from {$gml['board_file_table']} where bo_table = '$bo_table' ";
...
alert(__('Copy successful.'), './board_copy.php?bo_table='.$bo_table.'&amp;'.$qstr);
?>