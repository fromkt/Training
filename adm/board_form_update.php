...
if (!$_POST['gr_id']) { alert(__('You must select a group ID.')); }
if (!$bo_table) { alert(__('Be sure to enter the bulletin board TABLE name.')); }
if (!preg_match("/^([A-Za-z0-9_]{1,20})$/", $bo_table)) { alert(__('Board table available only letters, numbers, _ with no spaces. (20 characters)')); }
if (!$_POST['bo_subject']) { alert(__('Please enter a bulletin board title.')); }
...
$bo_include_head = preg_replace("#[\\\]+$#", "", substr($_POST['bo_include_head'], 0, 255));
$bo_include_tail = preg_replace("#[\\\]+$#", "", substr($_POST['bo_include_tail'], 0, 255));
...
$board_path = GML_DATA_PATH.'/file/'.$bo_table;

// 게시판 디렉토리 생성
@mkdir($board_path, GML_DIR_PERMISSION);
@chmod($board_path, GML_DIR_PERMISSION);
...
if ($w == '') {
    ...
    $create_table = $gml['write_prefix'] . $bo_table;
    ...
    // 게시판의 글 수
    $sql = " select count(*) as cnt from {$gml['write_prefix']}{$bo_table} where wr_is_comment = 0 ";
    $row = sql_fetch($sql);
    $bo_count_write = $row['cnt'];

    // 게시판의 코멘트 수
    $sql = " select count(*) as cnt from {$gml['write_prefix']}{$bo_table} where wr_is_comment = 1 ";
    $row = sql_fetch($sql);
    ...
    /*
        ...
        http://sir.kr/gml_tiptech/27207
    */
    if (isset($_POST['proc_count'])) {
        // 원글을 얻습니다.
        //$sql = " select wr_id from {$gml['write_prefix']}{$bo_table} where wr_is_comment = 0 ";
        $sql = " select a.wr_id, (count(b.wr_parent) - 1) as cnt from {$gml['write_prefix']}{$bo_table} a, {$gml['write_prefix']}{$bo_table} b where a.wr_id=b.wr_parent and a.wr_is_comment=0 group by a.wr_id ";
        $result = sql_query($sql);
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            /*
            // 코멘트수를 얻습니다.
            $sql2 = " select count(*) as cnt from {$gml['write_prefix']}$bo_table where wr_parent = '{$row['wr_id']}' and wr_is_comment = 1 ";
            $row2 = sql_fetch($sql2);
            */

            sql_query(" update {$gml['write_prefix']}{$bo_table} set wr_comment = '{$row['cnt']}' where wr_id = '{$row['wr_id']}' ");
        }
    }
    ...
    if ($board['bo_notice']) {
        ...
        for ($i=0; $i<count($tmp_array); $i++) {
            ...
            $row = sql_fetch(" select count(*) as cnt from {$gml['write_prefix']}{$bo_table} where wr_id = '{$tmp_wr_id}' ");
            if ($row['cnt'])
            {
...
if ($grp_fields) {
    sql_query(" update {$gml['board_table']} set bo_table = bo_table {$grp_fields} where gr_id = '$gr_id' ");
}
...
if ($all_fields) {
    sql_query(" update {$gml['board_table']} set bo_table = bo_table {$all_fields} ");
}
