...
if (!preg_match("/^([A-Za-z0-9_]{1,10})$/", $_POST['gr_id']))
    alert(__('Group ID available only letters, numbers, _ with no spaces. (10 characters)'));

if (!$gr_subject) alert(__('Please enter a group title.'));
...
if ($w == '') {
    ...
    $sql = " select count(*) as cnt from {$gml['group_table']} where gr_id = '{$_POST['gr_id']}' ";
    $row = sql_fetch($sql);
    if ($row['cnt'])
        alert(__('Group ID already exists.'));

    $sql = " insert into {$gml['group_table']}
