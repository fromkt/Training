...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($brow == 'etc') { $brow = '<span title="'.get_text($row['vi_agent']).'">'.$brow.'</span>'; }
        if ($os == 'etc') { $os = '<span title="'.get_text($row['vi_agent']).'">'.$os.'</span>'; }
    ...
    if ($i == 0)
        echo '<tr><td colspan="'.$colspan.'" class="empty_table">'.__('No data found or deleted by administrator.').'</td></tr>';
    ?>
    </tbody>
    </table>
</div>
