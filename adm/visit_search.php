...
include_once(GML_PATH.'/lib/visit.lib.php');

auth_check($auth[$sub_menu], 'r');

$gml['title'] = __('Visitor search');
include_once('./admin.head.php');
include_once(GML_PLUGIN_PATH.'/jquery-ui/datepicker.php');

$colspan = 6;
$listall = '<a href="'.$_SERVER['SCRIPT_NAME'].'" class="ov_listall">'.__('All search').'</a>';
?>
...
<div class="local_sch local_sch01">
    ...
    <label for="sch_sort" class="sound_only"><?php e__('Search classification'); ?></label>
    <select name="sfl" id="sch_sort" class="search_sort">
        <option value="vi_ip"<?php echo get_selected($sfl, 'vi_ip'); ?>>IP</option>
        <option value="vi_referer"<?php echo get_selected($sfl, 'vi_referer'); ?>><?php e__('Referer url'); ?></option>
        <option value="vi_date"<?php echo get_selected($sfl, 'vi_date'); ?>><?php e__('Date'); ?></option>
    </select>
    <label for="sch_word" class="sound_only"><?php e__('Search Word'); ?></label>
...
<div class="tbl_wrap tbl_head01">
    ...
    <tr>
        <th scope="col">IP</th>
        <th scope="col"><?php e__('Referer url'); ?></th>
        <th scope="col"><?php e__('Browser'); ?></th>
        <th scope="col">OS</th>
        <th scope="col"><?php e__('User agent'); ?></th>
        <th scope="col"><?php e__('Date'); ?></th>
    </tr>
    </thead>
    <tbody>
    <?php
    $sql_common = " from {$gml['visit_table']} ";
    if ($sfl) {
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if ($is_admin == 'super')
            ...
            $ip = preg_replace("/([0-9]+).([0-9]+).([0-9]+).([0-9]+)/", GML_IP_DISPLAY, $row['vi_ip']);

        $bg = 'bg'.($i%2);
    ?>
    <tr class="<?php echo $bg; ?>">
        <td class="td_id"><a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?sfl=vi_ip&amp;stx=<?php echo $ip; ?>"><?php echo $ip; ?></a></td>
        <td class="td_left"><?php echo $link.$title; ?></a></td>
    ...
    <?php } ?>
    <?php if ($i == 0) echo '<tr><td colspan="'.$colspan.'" class="empty_table">'.__('No Data').'</td></tr>'; ?>
    </tbody>
    </table>
</div>
