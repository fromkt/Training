...
<button type="button" class="cmt_btn"><?php e__('List of comments'); ?></button>
...
<section id="bo_vc">
    ...
    <article id="c_<?php echo $list[$i]['comment_id'] ?>" <?php if ($list[$i]['cmt_depth']) { ?>style="padding-left:<?php echo $list[$i]['cmt_depth'] ?>px;border-top-color:#e0e0e0" class="cmt_depth"<?php } ?>>
        <header>
            <h2><?php echo sprintf($list[$i]['cmt_depth'] ? __('Comment on comments by %s') : __('Comment by %s') , $list[$i]['wr_name']); ?></h2>
            <span class="comment_profile_img"><?php echo $list[$i]['get_member_profile_img'] ?></span> <?php echo $list[$i]['name'] ?>
            ...
            <?php include(GML_SNS_PATH."/view_comment_list.sns.skin.php"); ?>
        </header>
        ...
        <span id="edit_<?php echo $list[$i]['comment_id'] ?>"></span><!-- Edit -->
        <span id="reply_<?php echo $list[$i]['comment_id'] ?>"></span><!-- Reply -->
        <input type="hidden" id="secret_comment_<?php echo $list[$i]['comment_id'] ?>" value="<?php echo strstr($list[$i]['wr_option'],"secret") ?>">
        <textarea id="save_comment_<?php echo $list[$i]['comment_id'] ?>" style="display:none"><?php echo get_text($list[$i]['content1'], 0) ?></textarea>
		<div class="bo_vl_opt">
			<button class="cmt_opt"><span class="sound_only"></span><i class="fa fa-ellipsis-v" aria-hidden="true"></i></button>
...
<aside id="bo_vc_w">
    ...
    <textarea id="wr_content" name="wr_content" required title="<?php e__('Comments'); ?>"
    <?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?> placeholder="<?php e__('Enter comments'); ?>"><?php echo $c_wr_content; ?></textarea>

	<div class="bo_vc_w_info">
        <?php if ($is_guest) { ?>
        <label for="wr_name" class="sound_only"><?php e__('Name'); ?><strong> <?php e__('Required'); ?></strong></label>
        <input type="text" name="wr_name" value="<?php echo $get_cookie_sns_name ?>" id="wr_name" required class="frm_input required" size="25" placeholder="이름">
