...
if (count($list) == 0) {
    $no_notices = '<tr class="no_arm">
        			<td colspan="5">'.__('No notices found').'</td>
        		   </tr>';
}
...
<span class="arm_notice"><?php e__('Remove notifications') ?> - <b><?php echo sprintf(n__('%s day', '%s days', $config['cf_notice_del']), $config['cf_notice_del']) ?></b></span>
...
<div id="alarm">
   ...
   <div class="arm_btn">
       ...
       <div class="arm_btn_right">
           <button type="button" onclick="location.href='./notice.php'" <?php echo notice_btn_on_class($read, '') ?>><?php e__('View all') ?></button>
           <button type="button" onclick="location.href='./notice.php?read=y'" <?php echo notice_btn_on_class($read, 'y') ?>><?php e__('Readed') ?></button>
           <button type="button" onclick="location.href='./notice.php?read=n'" <?php echo notice_btn_on_class($read, 'n') ?>><?php e__('Unreaded') ?></button>
       </div>
   </div>
   <table>
       ...
       <tr>
           <td class="td_chk li_chk">
               <label for="chk_no_id_<?php echo $i ?>" class="sound_only"><?php e__('Select notice') ?></label>
               <input type="checkbox" name="chk_no_id[]" value="<?php echo $list[$i]['no_id'] ?>" id="chk_no_id_<?php echo $i ?>" />
           </td>
           <td class="td_stat <?php echo $list[$i]['read_class'] ?>"><i class="fa fa-circle"></i> <?php echo $list[$i]['status'] ?></td>
           <td class="td_tit"><?php echo $list[$i]['subject'] ?></td>
           <td class="td_time"><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $list[$i]['notice_datetime']; ?></td>
           <td class="td_del"><button class="list_del"><span class="sound_only"><?php e__('Delete') ?></span><i class="fa fa-times" aria-hidden="true"></i></button></td>
       </tr>
       <?php } ?>
...
$(document).ready(function(){
   ...
   $(".td_tit a").click(function(event){
       ...
       var read_class = $(this).parents("tr").find(".read_arm").text();
       var onclick = $(this).attr('onclick');
       if($.trim(read_class).length > 0) {
           var no_id = $(this).parents("tr").find("input[type=checkbox]").val();
