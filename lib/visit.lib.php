...
function visit($skin_dir='basic')
{
    ...
    // $visit[4] = 전체
    // 숫자가 들어감
    preg_match("/Today:(.*),Yesterday:(.*),Maximum:(.*),All:(.*)/", $config['cf_visit'], $visit);
    ...
    if (GML_IS_MOBILE) {
        $visit_skin_path = GML_THEME_MOBILE_PATH.'/'.GML_SKIN_DIR.'/visit/'.$skin_dir;
        if(!is_dir($visit_skin_path))
            $visit_skin_path = GML_THEME_PATH.'/'.GML_SKIN_DIR.'/visit/'.$skin_dir;
        $visit_skin_url = str_replace(GML_PATH, GML_URL, $visit_skin_path);
    } else {
        $visit_skin_path = GML_THEME_PATH.'/'.GML_SKIN_DIR.'/visit/'.$skin_dir;
        $visit_skin_url = str_replace(GML_PATH, GML_URL, $visit_skin_path);
    }
    ...
    //방문자 스킨 언어파일을 가져옵니다.
    bind_lang_domain( 'default', get_path_lang_dir('skin', $visit_skin_path.'/'.GML_LANG_DIR) );
