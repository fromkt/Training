...
function get_list_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if($row['bf_file']) {
        ...
        $filepath = GML_DATA_PATH.'/file/'.$bo_table;
        ...
        if( $matches = get_editor_image($write['wr_content'], false) ){
            for($i=0; $i<count($matches[1]); $i++)
            {
                ...
                if(strpos($p['path'], '/'.GML_DATA_DIR.'/') != 0)
                    $data_path = preg_replace('/^\/.*\/'.GML_DATA_DIR.'/', '/'.GML_DATA_DIR, $p['path']);
                else
                    $data_path = $p['path'];

                $srcfile = GML_PATH.$data_path;
    ...
    if($tname) {
        if($edt) {
            // 오리지날 이미지
            $ori = GML_URL.$data_path;
            // 썸네일 이미지
            $src = GML_URL.str_replace($filename, $tname, $data_path);
        } else {
            $ori = GML_DATA_URL.'/file/'.$bo_table.'/'.$filename;
            $src = GML_DATA_URL.'/file/'.$bo_table.'/'.$tname;
        }
    } else {
        return false;
    }
...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img src="'.GML_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img src="'.GML_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'"/>';
            }

            // $img_tag에 editor 경로가 있으면 원본보기 링크 추가
            $img_tag = $matches[0][$i];
            if(strpos($img_tag, GML_DATA_DIR.'/'.GML_EDITOR_DIR) && preg_match("/\.({$config['cf_image_extension']})$/i", $filename)) {
                $imgurl = str_replace(GML_URL, "", $src);
                $thumb_tag = '<a href="'.GML_BBS_URL.'/view_image.php?fn='.urlencode($imgurl).'" target="_blank" class="view_image">'.$thumb_tag.'</a>';
            }
...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if (!is_dir($target_path)) {
        @mkdir($target_path, GML_DIR_PERMISSION);
        @chmod($target_path, GML_DIR_PERMISSION);
    }
    ...
    if($is_large) {
        ...
        if($is_crop) {
            ...
            if ( !((defined('GML_USE_THUMB_RATIO') && false === GML_USE_THUMB_RATIO) || (defined('GML_THEME_USE_THUMB_RATIO') && false === GML_THEME_USE_THUMB_RATIO)) ){
                if($src_w > $src_h) {
    ...
    if($size[2] == 1) {
        ...
        if(!defined('GML_THUMB_PNG_COMPRESS'))
            $png_compress = 5;
        else
            $png_compress = GML_THUMB_PNG_COMPRESS;

        imagepng($dst, $thumb_file, $png_compress);
    } else {
        if(!defined('GML_THUMB_JPG_QUALITY'))
            $jpg_quality = 90;
        else
            $jpg_quality = GML_THUMB_JPG_QUALITY;

        imagejpeg($dst, $thumb_file, $jpg_quality);
    }

    chmod($thumb_file, GML_FILE_PERMISSION); // 추후 삭제를 위하여 파일모드 변경
